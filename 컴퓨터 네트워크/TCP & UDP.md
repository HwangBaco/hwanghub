...
네트워크 프로토콜이란 두 컴퓨터 간에 데이터를 주고받는 방법을 합의한 것이라고 이해할 수 있다. 우리는 그 중에서 TCP와 UDP에 대하여 주요하게 다룬다. 이 것들이 바로 "두 컴퓨터가 어떻게 데이터를 주고받는지"를 정한 규약, 즉 데이터 전송 계층의 프로토콜이기 때문이다.
...
대표적인 TCP 활용 프로토콜로는 HTTP/1.1, HTTP/2가 있다. (HTTP/3은 UDP를 활용한다고 한다.)
